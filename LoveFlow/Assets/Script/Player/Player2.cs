    ...
    void InputAction()
    {
        switch (_mode)
        {
            case GameMode.PLAYMODE:
                {
                    ...
                    // 캐릭터 회전 왼쪽
                    if (Input.GetAxis(_joyStickNum + "_RightVertical") < -0.05 || Input.GetKey(KeyCode.RightControl))
                    {
                        Vector4 Lookvector = new Vector3(_CameraTM.transform.position.x, 0.0f, _CameraTM.transform.position.z);
                        moveDirection = -Lookvector * _playerSpeed * Time.deltaTime;
                        //transform.Rotate(0, -40.0f * Time.deltaTime, 0);
                    }
                    if (Input.GetAxis(_joyStickNum + "_RightVertical") > 0.05 || Input.GetKey(KeyCode.Keypad0))
                    {
                        Vector4 Lookvector = new Vector3(_CameraTM.transform.position.x, 0.0f, _CameraTM.transform.position.z);
                        moveDirection = -Lookvector * _playerSpeed * Time.deltaTime;
                    ...
                    // 아이템
                    if (Input.GetButtonDown(_joyStickNum + "_XButton") && _buttonOn == true || Input.GetKey(KeyCode.Backslash))
                    {
                        // 망치 줍기
                    ...
                    if (_selectCube != null && Input.GetButtonDown(_joyStickNum + "_BButton") && _selectCube.CompareTag("Cube")
                        || Input.GetKeyDown(KeyCode.Keypad4) && _selectCube != null && _selectCube.CompareTag("Cube"))
                    {
                        if (_buttonOn == true)
                        {
            ...
            case GameMode.INSTALLMODE:
                {
                    ...
                    if (Input.GetButtonDown(_joyStickNum + "_AButton") || Input.GetKeyDown(KeyCode.Keypad5))
                    {
                    ...
                    //파이프 모양 선택
                    if (Input.GetAxis(_joyStickNum + "_LeftTrigger") == 1 || Input.GetKeyDown(KeyCode.KeypadEnter))
                    {
                        if (_buttonOn == true)
                        {
                    ...
                    // 파이프 회전
                    if (Input.GetAxis(_joyStickNum + "_DpadHorizontal") == 1 || Input.GetKeyDown(KeyCode.Keypad1))
                    {
                        if (_buttonOn == true)
                        {
                    ...
                    if (Input.GetAxis(_joyStickNum + "_DpadHorizontal") == -1 || Input.GetKeyDown(KeyCode.Keypad2))
                    {
                        if (_buttonOn == true)
                        {
                            SoundManager.Instance.PlayEffect(SoundManager.ClipEffect.Install_mode_Pipe_Turn);
                    ...
                    if (Input.GetAxis(_joyStickNum + "_DpadVertical") == 1 || Input.GetKeyDown(KeyCode.Keypad3))
                    {
                        if (_buttonOn == true)
                        {
                            SoundManager.Instance.PlayEffect(SoundManager.ClipEffect.Install_mode_Pipe_Turn);
                    ...
                    if ((Input.GetButtonUp(_joyStickNum + "_BButton") && (_buttonOn == true) && _selectCube._childObject.GetComponent<Pipe>()._currentObject >= 1)
                        || Input.GetKeyDown(KeyCode.Keypad5) && _selectCube._childObject.GetComponent<Pipe>()._currentObject >= 1)
                    {
                        _buttonOn = false;
                        _selectCube._isInstalled = true;
                        _selectCube = null;
            ...
            case GameMode.DESTROYMODE:
                {
                    if ((Input.GetButtonUp(_joyStickNum + "_BButton") && _buttonOn == true)
